...
{
    if (report == NULL || !tud_hid_n_ready(MBR_USB_HID_MOUSE_INTERFACE)) return false;
    return tud_hid_n_report(
        MBR_USB_HID_MOUSE_INTERFACE,
        0u,
        report,
...
{
    if (report == NULL || !tud_hid_n_ready(MBR_USB_HID_KEYBOARD_INTERFACE)) return false;
    return tud_hid_n_report(
        MBR_USB_HID_KEYBOARD_INTERFACE,
        0u,
        report,
        (uint16_t)sizeof(*report));
}
