...
static const tusb_desc_device_t k_device_descriptor = {
    ...
    .idVendor = MBR_USB_HID_VID,
    .idProduct = MBR_USB_HID_PID,
    .bcdDevice = MBR_USB_HID_BCD_DEVICE,
    .iManufacturer = 0x01,
    .iProduct = 0x02,
    .iSerialNumber = 0x00,
...
{
    if (instance == MBR_USB_HID_MOUSE_INTERFACE) return k_mouse_report_descriptor;
    if (instance == MBR_USB_HID_KEYBOARD_INTERFACE) return k_keyboard_report_descriptor;
    return NULL;
}

enum {
    ITF_NUM_MOUSE = MBR_USB_HID_MOUSE_INTERFACE,
    ITF_NUM_KEYBOARD = MBR_USB_HID_KEYBOARD_INTERFACE,
    ITF_NUM_TOTAL = MBR_USB_HID_INTERFACE_COUNT,
};
...
const uint16_t *tud_descriptor_string_cb(uint8_t index, uint16_t langid)
{
    ...
    if (index == 1u) text = MBR_USB_HID_MANUFACTURER;
    if (index == 2u) text = MBR_USB_HID_PRODUCT;
    if (text == NULL) return NULL;

    size_t count = strlen(text);
    const size_t maximum = (sizeof(k_string_descriptor) / sizeof(k_string_descriptor[0])) - 1u;
