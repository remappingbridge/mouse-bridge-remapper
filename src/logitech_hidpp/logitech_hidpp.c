...
{
    return id == MBR_HIDPP_REPORT_ID_SHORT || id == MBR_HIDPP_REPORT_ID_LONG;
}
...
static void build_get_feature(mbr_hidpp_output_t *output)
{
    memset(output, 0, sizeof(*output));
    output->kind = MBR_HIDPP_OUTPUT_GET_FEATURE;
    output->report_id = MBR_HIDPP_REPORT_ID_LONG;
    output->payload_len = MBR_HIDPP_LONG_PAYLOAD_SIZE;
    output->payload[0] = HIDPP_DEVICE_INDEX;
    output->payload[1] = HIDPP_ROOT_FEATURE_INDEX;
    output->payload[2] = HIDPP_SOFTWARE_ID;
    output->payload[3] = (uint8_t)(MBR_HIDPP_REPROG_CONTROLS_V4 >> 8u);
    output->payload[4] = (uint8_t)(MBR_HIDPP_REPROG_CONTROLS_V4 & 0xffu);
}

static void build_set_forward(const mbr_logitech_hidpp_t *hidpp,
                              bool enable,
                              mbr_hidpp_output_t *output)
{
    memset(output, 0, sizeof(*output));
    output->kind = MBR_HIDPP_OUTPUT_SET_FORWARD_DIVERT;
    output->report_id = MBR_HIDPP_REPORT_ID_LONG;
    output->payload_len = MBR_HIDPP_LONG_PAYLOAD_SIZE;
    output->payload[0] = HIDPP_DEVICE_INDEX;
    output->payload[1] = hidpp->feature_index;
    output->payload[2] = (uint8_t)((HIDPP_SET_CONTROL_REPORTING_FUNCTION << 4u) | HIDPP_SOFTWARE_ID);
    output->payload[3] = (uint8_t)(MBR_HIDPP_FORWARD_CID >> 8u);
    output->payload[4] = (uint8_t)(MBR_HIDPP_FORWARD_CID & 0xffu);
                                         ...
                                         mbr_hidpp_output_t *output)
{
    if (hidpp == NULL || output == NULL || !hidpp->connected ||
        hidpp->waiting_for != MBR_HIDPP_OUTPUT_NONE) return false;
    if (hidpp->feature_index == 0u) {
        if (!hidpp->forward_desired || hidpp->feature_failed) return false;
        build_get_feature(output);
        return true;
    }
    if (hidpp->forward_applied == hidpp->forward_desired ||
        (hidpp->feature_failed && hidpp->forward_desired)) return false;
...
void mbr_logitech_hidpp_output_result(mbr_logitech_hidpp_t *hidpp,
                                           ...
                                           bool accepted)
{
    if (hidpp != NULL && accepted && kind != MBR_HIDPP_OUTPUT_NONE)
                                           ...
                                           mbr_hidpp_input_result_t *result)
{
    ...
    const bool interested = hidpp->forward_desired || hidpp->forward_applied ||
        hidpp->waiting_for != MBR_HIDPP_OUTPUT_NONE || hidpp->feature_index != 0u;
    if (!interested) return false;
    if (result != NULL) result->consumed = true;

    if (hidpp->waiting_for == MBR_HIDPP_OUTPUT_GET_FEATURE) {
        if (error_matches(payload, payload_len, HIDPP_ROOT_FEATURE_INDEX, 0u)) {
            hidpp->feature_failed = true;
            hidpp->waiting_for = MBR_HIDPP_OUTPUT_NONE;
            return true;
        }
    ...
    if (hidpp->waiting_for == MBR_HIDPP_OUTPUT_SET_FORWARD_DIVERT) {
    ...
    if (hidpp->feature_index != 0u && payload_len >= 3u &&
        ...
        for (size_t pos = 3u; pos + 1u < payload_len; pos += 2u) {
            ...
            if (cid == 0u) break;
            if (cid == MBR_HIDPP_FORWARD_CID) held = true;
        }
