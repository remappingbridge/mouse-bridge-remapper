/* Physical ST7789 initialization adapted from accepted G06. */
...
static bool valid_rect(uint16_t x,uint16_t y,uint16_t width,uint16_t height) { return width>0u&&height>0u&&x<MBR_LCD_WIDTH&&y<MBR_LCD_HEIGHT&&(uint32_t)x+width<=MBR_LCD_WIDTH&&(uint32_t)y+height<=MBR_LCD_HEIGHT; }
...
void mbr_display_backlight(bool enabled) { gpio_put(LCD_PIN_BL,enabled); }
...
bool mbr_display_init(void) { (void)spi_init(LCD_SPI,LCD_SPI_BAUD_HZ); gpio_set_function(LCD_PIN_SCK,GPIO_FUNC_SPI); gpio_set_function(LCD_PIN_MOSI,GPIO_FUNC_SPI); const uint outputs[]={LCD_PIN_DC,LCD_PIN_CS,LCD_PIN_RST,LCD_PIN_BL}; for(size_t i=0u;i<sizeof(outputs)/sizeof(outputs[0]);++i){gpio_init(outputs[i]);gpio_set_dir(outputs[i],GPIO_OUT);} gpio_put(LCD_PIN_CS,true);gpio_put(LCD_PIN_DC,false);gpio_put(LCD_PIN_RST,true);mbr_display_backlight(false);sleep_ms(100u);gpio_put(LCD_PIN_RST,false);sleep_ms(100u);gpio_put(LCD_PIN_RST,true);sleep_ms(100u);configure_panel();return true; }

void mbr_display_rows(unsigned y,unsigned count,const uint16_t *pixels) { (void)pico_write_rgb565(NULL,0,(uint16_t)y,240,(uint16_t)count,pixels); }
