...
{
    uint32_t value = 0u;
    for (size_t index = 0u; index < size; ++index) value |= ((uint32_t)data[index]) << (8u * index);
    return value;
}
...
static int32_t item_signed(const uint8_t *data, size_t size)
{
    ...
    if (bits < 32u && (value & (UINT32_C(1) << (bits - 1u))) != 0u) value |= UINT32_MAX << bits;
...
static MbrHogpReport *find_report(MbrHogpParser *parser,
                                                     uint8_t report_id,
                                                     bool create)
{
    for (size_t index = 0u; index < parser->report_count; ++index) {
        if (parser->reports[index].report_id == report_id) return &parser->reports[index];
    }
    if (!create || parser->report_count >= MBR_HOGP_MAX_REPORTS) return NULL;
    MbrHogpReport *report = &parser->reports[parser->report_count++];
    memset(report, 0, sizeof(*report));
    report->report_id = report_id;
    return report;
}

static const MbrHogpReport *find_report_const(const MbrHogpParser *parser,
                                                                 uint8_t report_id)
{
...
static bool add_field(MbrHogpParser *parser,
                      uint8_t report_id,
                      MbrHogpFieldKind kind,
                      uint16_t bit_offset,
                      uint8_t bit_size,
                      uint8_t button_index,
                      bool signed_value)
{
    if (parser->field_count >= MBR_HOGP_MAX_FIELDS || bit_size == 0u || bit_size > 32u) return false;
    MbrHogpField *field = &parser->fields[parser->field_count++];
...
static bool classify_and_add(MbrHogpParser *parser,
                             ...
                             uint32_t input_flags)
{
    ...
    if (page == HID_USAGE_PAGE_BUTTON && code >= 1u && code <= 5u) {
        return add_field(parser, globals->report_id, MBR_HOGP_FIELD_BUTTON,
                         bit_offset, globals->report_size, (uint8_t)(code - 1u), false);
    }
    if (!relative) return true;

    if (page == HID_USAGE_PAGE_GENERIC_DESKTOP) {
        if (code == HID_USAGE_X) return add_field(parser, globals->report_id, MBR_HOGP_FIELD_X,
                                                   bit_offset, globals->report_size, 0u, globals->logical_minimum < 0);
        if (code == HID_USAGE_Y) return add_field(parser, globals->report_id, MBR_HOGP_FIELD_Y,
                                                   bit_offset, globals->report_size, 0u, globals->logical_minimum < 0);
        if (code == HID_USAGE_WHEEL) return add_field(parser, globals->report_id, MBR_HOGP_FIELD_WHEEL,
                                                       bit_offset, globals->report_size, 0u, globals->logical_minimum < 0);
    }
    if (page == HID_USAGE_PAGE_CONSUMER && code == HID_USAGE_AC_PAN) {
        return add_field(parser, globals->report_id, MBR_HOGP_FIELD_PAN,
                         bit_offset, globals->report_size, 0u, globals->logical_minimum < 0);
    }
    return true;
}

bool mbr_hogp_configure(MbrHogpParser *parser,
                                        MouseSessionId source,
                                        const uint8_t *descriptor,
                                        size_t descriptor_len)
{
    if (parser == NULL) return false;
    memset(parser, 0, sizeof(*parser));
    if (descriptor == NULL || descriptor_len == 0u || source == 0) return false;
    parser->session = source;
    ...
    while (position < descriptor_len) {
        ...
        if (item_type == 2u) {
            ...
            case HID_LOCAL_USAGE:
                ...
                break;
            case HID_LOCAL_USAGE_MINIMUM: locals.usage_minimum = usage; locals.has_usage_range = true; break;
            case HID_LOCAL_USAGE_MAXIMUM: locals.usage_maximum = usage; locals.has_usage_range = true; break;
            default: break;
            }
            continue;
        }
        ...
        if (item_tag == HID_MAIN_INPUT) {
            MbrHogpReport *report = find_report(parser, globals.report_id, true);
            ...
            if (in_mouse_collection && (unsigned_value & HID_INPUT_CONSTANT) == 0u &&
                ...
                for (uint32_t index = 0u; index < globals.report_count; ++index) {
                    ...
                    if (!classify_and_add(parser, &globals, usage, (uint16_t)field_offset, unsigned_value)) return false;
                }
            }
    ...
    if (collection_depth != 0u || global_depth != 0u) return false;
    parser->configured = parser->field_count > 0u;
    return parser->configured;
}

bool mbr_hogp_has_mouse(const MbrHogpParser *parser)
{
    return parser != NULL && parser->configured && parser->field_count > 0u;
}

bool mbr_hogp_normalize(const MbrHogpParser *parser,
                                               uint8_t report_id,
                                               ...
                                               size_t *payload_len)
{
    ...
    const MbrHogpReport *state = find_report_const(parser, report_id);
...
static bool extract_value(const uint8_t *report,
                          size_t report_len,
                          uint16_t bit_offset,
                          uint8_t bit_size,
                          bool signed_value,
                          int32_t *out_value)
{
    if (report == NULL || out_value == NULL || bit_size == 0u || bit_size > 32u) return false;
    if ((size_t)bit_offset + (size_t)bit_size > report_len * 8u) return false;
    uint32_t raw = 0u;
    for (uint8_t index = 0u; index < bit_size; ++index) {
        const size_t bit = (size_t)bit_offset + index;
        if ((report[bit >> 3u] & (uint8_t)(1u << (bit & 7u))) != 0u) raw |= UINT32_C(1) << index;
    }
    if (signed_value && bit_size < 32u && (raw & (UINT32_C(1) << (bit_size - 1u))) != 0u) raw |= UINT32_MAX << bit_size;
...
static bool emit_button_deltas(MbrHogpParser *parser,
                               uint8_t old_mask,
                               uint8_t new_mask,
                               MbrHogpEmit emit,
                               void *context)
{
    ...
    for (uint8_t button = 0u; button < 5u; ++button) {
        const uint8_t mask = (uint8_t)(1u << button);
        if ((changed & mask) == 0u) continue;
        MbrMouseEvent event = {0};
        event.session = parser->session;
        event.type = MBR_MOUSE_BUTTON;
        event.data.button.button = (uint8_t)button;
...
static bool parse_payload(MbrHogpParser *parser,
                          uint8_t report_id,
                          const uint8_t *report,
                          size_t report_len,
                          MbrHogpEmit emit,
                          void *context)
{
    MbrHogpReport *report_state = find_report(parser, report_id, false);
    if (report_state == NULL) return false;

    uint8_t candidate_buttons = report_state->button_mask;
    bool has_button_fields = false;
    int64_t dx = 0, dy = 0, wheel = 0, pan = 0;

    for (size_t index = 0u; index < parser->field_count; ++index) {
        const MbrHogpField *field = &parser->fields[index];
        if (field->report_id != report_id) continue;
        int32_t value = 0;
        if (!extract_value(report, report_len, field->bit_offset, field->bit_size, field->signed_value, &value)) return false;
        switch (field->kind) {
        case MBR_HOGP_FIELD_BUTTON: {
        ...
        case MBR_HOGP_FIELD_X: dx += value; break;
        case MBR_HOGP_FIELD_Y: dy += value; break;
        case MBR_HOGP_FIELD_WHEEL: wheel += value; break;
        case MBR_HOGP_FIELD_PAN: pan += value; break;
        }
    }
    ...
    if (has_button_fields) {
        ...
        for (size_t index = 0u; index < parser->report_count; ++index) {
            new_aggregate = (uint8_t)(new_aggregate |
                (parser->reports[index].report_id == report_id ? candidate_buttons : parser->reports[index].button_mask));
        }
...
bool mbr_hogp_parse(MbrHogpParser *parser,
                                           uint8_t report_id,
                                           const uint8_t *report,
                                           size_t report_len,
                                           MbrHogpEmit emit,
                                           void *context)
{
    ...
    if (!mbr_hogp_normalize(parser, report_id, report, report_len, &payload, &payload_len)) return false;
