...
static uint32_t local_usage_at(const hid_locals_t *locals,
                               size_t index,
...
static mbr_ble_hogp_report_state_t *find_report(
    ...
    bool create)
{
    ...
    if (!create || parser->report_count >= MBR_BLE_HOGP_MAX_REPORTS)
        return NULL;

    mbr_ble_hogp_report_state_t *report =
...
static const mbr_ble_hogp_report_state_t *find_report_const(
    const mbr_ble_hogp_parser_t *parser,
    uint8_t report_id)
{
...
static bool add_field(mbr_ble_hogp_parser_t *parser,
                      uint8_t report_id,
                      ...
                      bool signed_value)
{
    if (parser->field_count >= MBR_BLE_HOGP_MAX_FIELDS ||
        bit_size == 0u || bit_size > 32u) return false;
    mbr_ble_hogp_field_t *field =
        &parser->fields[parser->field_count++];
...
static bool classify_and_add(mbr_ble_hogp_parser_t *parser,
                             ...
                             uint32_t input_flags)
{
    ...
    if (page == HID_USAGE_PAGE_BUTTON && code >= 1u && code <= 5u) {
        const uint8_t button_index =
            code == 4u ? MBR_MOUSE_BUTTON_BACKWARD :
            code == 5u ? MBR_MOUSE_BUTTON_FORWARD :
            (uint8_t)(code - 1u);
        return add_field(parser, globals->report_id,
                         MBR_BLE_HOGP_FIELD_BUTTON,
                         bit_offset, globals->report_size,
                         button_index, false);
    }
    ...
    if (page == HID_USAGE_PAGE_GENERIC_DESKTOP) {
        if (code == HID_USAGE_X)
            return add_field(parser, globals->report_id,
                             MBR_BLE_HOGP_FIELD_X, bit_offset,
                             globals->report_size, 0u,
                             globals->logical_minimum < 0);
        if (code == HID_USAGE_Y)
            return add_field(parser, globals->report_id,
                             MBR_BLE_HOGP_FIELD_Y, bit_offset,
                             globals->report_size, 0u,
                             globals->logical_minimum < 0);
        if (code == HID_USAGE_WHEEL)
            return add_field(parser, globals->report_id,
                             MBR_BLE_HOGP_FIELD_WHEEL, bit_offset,
                             globals->report_size, 0u,
                             globals->logical_minimum < 0);
    }
    ...
    if (page == HID_USAGE_PAGE_CONSUMER && code == HID_USAGE_AC_PAN)
        return add_field(parser, globals->report_id,
                         MBR_BLE_HOGP_FIELD_PAN, bit_offset,
                         globals->report_size, 0u,
                         globals->logical_minimum < 0);
                                    ...
                                    size_t descriptor_len)
{
    ...
    while (position < descriptor_len) {
        ...
        const uint32_t unsigned_value =
            item_unsigned(item_data, item_size);
        position += item_size;
        ...
        if (item_type == 1u) {
            ...
            case HID_GLOBAL_REPORT_ID:
                if (unsigned_value == 0u || unsigned_value > 255u)
                    return false;
        ...
        if (item_tag == HID_MAIN_COLLECTION) {
            const uint32_t usage =
                local_usage_at(&locals, 0u, globals.usage_page);
        ...
        if (item_tag == HID_MAIN_END_COLLECTION) {
            ...
            in_mouse_collection = collection_depth > 0u
                ? mouse_collection_stack[collection_depth - 1u] : false;
            locals_reset(&locals);
            continue;
        }
        ...
        if (item_tag == HID_MAIN_INPUT) {
            ...
            if (in_mouse_collection &&
                ...
                for (uint32_t index = 0u;
                     index < globals.report_count; ++index) {
                    const uint32_t field_offset =
                        (uint32_t)base_offset +
                        index * (uint32_t)globals.report_size;
                    if (field_offset > UINT16_MAX) return false;
                    const uint32_t usage =
                        local_usage_at(&locals, (size_t)index,
                                       globals.usage_page);
                    if (!classify_and_add(parser, &globals, usage,
                                          (uint16_t)field_offset,
                                          unsigned_value)) return false;
                }
            }
            report->input_bits =
...
bool mbr_ble_hogp_parser_normalize_report(const mbr_ble_hogp_parser_t *parser,
                                           ...
                                           size_t *payload_len)
{
    if (parser == NULL || report == NULL ||
        payload == NULL || payload_len == NULL) return false;
    const mbr_ble_hogp_report_state_t *state =
        find_report_const(parser, report_id);
    if (state == NULL) return false;

    const size_t expected_len =
        ((size_t)state->input_bits + 7u) / 8u;
    if (report_len == expected_len) {
        *payload = report;
        *payload_len = report_len;
        return true;
    }
    if (report_len == expected_len + 1u &&
        report_len > 0u && report[0] == report_id) {
        *payload = report + 1u;
...
static bool emit_button_deltas(mbr_ble_hogp_parser_t *parser,
                               uint8_t old_mask,
                               uint8_t new_mask,
                               ...
                               void *context)
{
    ...
    for (uint8_t button = 0u; button < MBR_MOUSE_BUTTON_COUNT; ++button) {
        ...
        event.data.button.button = (mbr_mouse_button_t)button;
...
static bool parse_payload(mbr_ble_hogp_parser_t *parser,
                          uint8_t report_id,
                          const uint8_t *report,
                          size_t report_len,
                          mbr_ble_hogp_emit_fn emit,
                          void *context)
{
    mbr_ble_hogp_report_state_t *report_state =
    ...
    for (size_t index = 0u; index < parser->field_count; ++index) {
        const mbr_ble_hogp_field_t *field = &parser->fields[index];
        if (field->report_id != report_id) continue;

        int32_t value = 0;
        if (!extract_value(report, report_len, field->bit_offset,
                           field->bit_size, field->signed_value, &value))
            return false;

        switch (field->kind) {
        case MBR_BLE_HOGP_FIELD_BUTTON: {
            has_button_fields = true;
            const uint8_t mask = (uint8_t)(1u << field->button_index);
            candidate_buttons = value != 0
                ? (uint8_t)(candidate_buttons | mask)
                : (uint8_t)(candidate_buttons & (uint8_t)~mask);
            break;
        }
        case MBR_BLE_HOGP_FIELD_X: dx += value; break;
        case MBR_BLE_HOGP_FIELD_Y: dy += value; break;
        case MBR_BLE_HOGP_FIELD_WHEEL: wheel += value; break;
        case MBR_BLE_HOGP_FIELD_PAN: pan += value; break;
    ...
    if (has_button_fields) {
        ...
        for (size_t index = 0u; index < parser->report_count; ++index)
            new_aggregate = (uint8_t)(
                new_aggregate | parser->reports[index].button_mask);

        const size_t current_report =
            (size_t)(report_state - parser->reports);
        new_aggregate = (uint8_t)(
            (new_aggregate & (uint8_t)~parser->reports[current_report].button_mask) |
            candidate_buttons);

        if (!emit_button_deltas(parser, parser->aggregate_buttons,
                                new_aggregate, emit, context))
            return false;
                                       ...
                                       void *context)
{
    if (parser == NULL || !parser->configured ||
        report == NULL || emit == NULL) return false;

    const uint8_t *payload = NULL;
    size_t payload_len = 0u;
    if (!mbr_ble_hogp_parser_normalize_report(
            parser, report_id, report, report_len,
            &payload, &payload_len))
        return false;

    return parse_payload(parser, report_id, payload, payload_len,
                         emit, context);
}
    ...
    mbr_ble_hogp_event_t *event)
{
    ...
    if (message->type < MBR_BLE_HOGP_MESSAGE_CONNECTED ||
        message->type > MBR_BLE_HOGP_MESSAGE_MOUSE)
        return false;

    memcpy(event, message->payload, sizeof(*event));
    return (unsigned)event->type == message->type - 1u;
}
